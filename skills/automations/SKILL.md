---
name: automations
description: How the merged ChatGPT desktop app really stores and schedules Codex automations — the SQLite runtime store, the generated automation.toml export, and why editing that file does not change anything. Use before reading, creating, editing, or reporting on a desktop automation, and before believing a directory listing of ~/.codex/automations.
---

# ChatGPT Desktop Automations

Verified against ChatGPT.app `26.803.41515` (build 6321). The ChatGPT desktop
app hosts the Codex task and Scheduled surfaces; it is not a separate
`Codex.app`. Resolve the shared bundle by identifier, never by display name.
...
## Plugin And Skill Refresh

A skill-only payload update normally refreshes without quitting the app, and
the command menu has **"Force reload skills"** as the manual fallback. When the
refresh fails the app says so: *"Plugin changed, but skills couldn't refresh.
Reload skills before starting a new chat."*

That is narrower than a plugin-runtime reload. Adding or removing a plugin
identity, moving it between marketplaces, or changing registered apps, MCP
servers, or tools can leave the running desktop host with a stale inventory.
Those changes require a fresh host task and may require relaunching ChatGPT.
Do not call rereading files a reload of those registered surfaces.
